        ...
        {open ? (
          <>
            ...
            <motion.aside
              ...
              aria-modal="true"
              className="fixed inset-y-0 left-0 z-50 flex w-[min(22rem,calc(100vw-2rem))] flex-col border-r border-outline-variant/50 bg-surface-container-low p-4 shadow-xl lg:hidden"
              initial={{ x: "-100%" }}
              animate={{ x: 0 }}
              exit={{ x: "-100%" }}
              role="dialog"
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <Link className="flex items-center gap-3" href="/student" onClick={() => setOpen(false)}>
                  <span className="flex size-9 items-center justify-center rounded-lg bg-primary text-primary-foreground">
                    <GraduationCap className="size-5" />
                  </span>
                  <span>
                    <span className="block text-base font-extrabold text-primary">EthioPrep AI</span>
              ...
              <div className="mt-5 rounded-lg border border-outline-variant/50 bg-surface-container-lowest p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div key={course.id} className="space-y-3">
                      ...
                      {course.units.map((unit) => (
                        <div key={unit.id} className="space-y-1">
                          ...
                          {unit.concepts.slice(0, 6).map((concept) => (
                            <Link
                              key={concept.id}
                              className={cn(
