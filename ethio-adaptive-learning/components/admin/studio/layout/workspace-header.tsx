...
import { Bell, ChevronRight, History, Network, Menu, Search } from "lucide-react"
...
export function WorkspaceHeader({ 
  ...
  const { setIsMobileNexusOpen, setIsContextMobileOpen } = useStudioLayout()
    ...
    <header className="bg-surface-container-lowest border-b border-outline-variant h-16 flex justify-between items-center px-4 lg:px-8 sticky top-0 z-30 shadow-sm shrink-0">
      <div className="flex items-center gap-2 lg:gap-6 min-w-0">
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button
            ...
            onClick={() => setIsContextMobileOpen(true)}
        ...
        <div className="flex items-center gap-4 min-w-0">
          <h1 className="font-display text-sm lg:text-lg font-bold text-on-surface truncate hidden sm:block">
            {title}
          </h1>
          ...
          {breadcrumbs.length > 0 && (
            <>
              ...
              <nav className="flex items-center gap-1.5 text-xs font-medium min-w-0 overflow-hidden">
                {breadcrumbs.map((crumb, i) => (
                  <React.Fragment key={i}>
                    {i > 0 && <ChevronRight className="size-3 text-outline-variant shrink-0" />}
                    <span className={cn(
                      "truncate",
                      i === breadcrumbs.length - 1 ? "text-on-surface" : "text-on-surface-variant opacity-60"
                    )}>
                      {crumb.label}
                    </span>
                  </React.Fragment>
      ...
      <div className="flex items-center gap-2 lg:gap-4 shrink-0">
        ...
        <div className="relative hidden xl:block">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 text-secondary size-[14px]" />
          <input
            className="pl-8 pr-10 py-1.5 bg-surface border border-outline-variant rounded-full text-xs w-48 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"
            placeholder="Search..."
            type="text"
          />
