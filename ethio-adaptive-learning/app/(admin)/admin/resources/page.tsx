...
function renderSnippetPreview(blocks: Array<{ type: string; text?: string; title?: string }>): string {
  return blocks
    .map((block) => {
      if (block.title) {
        return `**${block.title}**`
      ...
      // Detect code blocks for minimal highlighting
      if (text.includes("```")) {
        return `\`\`\`\n${text}\n\`\`\``
      }
      
      // Detect latex for math
      if (text.includes("$")) {
        return `[Math]: ${text}`
      }
...
export default async function ResourcesPage() {
  ...
  const mediaAssetItems = await Promise.all(
  ...
  const snippetItems = contentSnippets.map((snippet) => {
    ...
    const preview = renderSnippetPreview(blocks)
  ...
  const unifiedItems: ResourceItem[] = [...mediaAssetItems, ...snippetItems]
